...
export class SessionCommandController {
  ...
  async selectAgents(request: SessionSelectAgentsRequest): Promise<SessionSelectAgentsValue> {
    const agent = await this.resolveAgent(request.sessionId)
    return this.agents.serializeImageAdmission(agent, () => {
      const selection = request.selection === null
        ? null
        : Object.fromEntries(Object.entries(request.selection).sort(([left], [right]) => left.localeCompare(right)))
      agent.session.append('agent/selection', { selection })
