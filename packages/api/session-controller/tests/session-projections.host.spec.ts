...
describe('session.history projections block', () => {
  ...
  it('projects the latest product-Agent selection for reload and restart recovery', async () => {
    const { ctx, session } = await harness(true)
    remote(ctx)
    await new Promise(resolve => setTimeout(resolve, 0))
    session.append('agent/selection', {
      selection: { codex: 'gpt-5.6-sol', qwen: 'qwen3.8-max' },
    })

    expect(ctx.sessionProjections.snapshot(session).values.agentSelection).toEqual({
      codex: 'gpt-5.6-sol',
      qwen: 'qwen3.8-max',
    })

    session.append('agent/selection', { selection: null })
    expect(ctx.sessionProjections.snapshot(session).values.agentSelection).toBeNull()
  })
