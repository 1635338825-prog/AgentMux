...
export class FakeApiClient {
  ...
  onSelectAgents: (payload: SessionSelectAgentsRequest) => Promise<RemoteResult<SessionSelectAgentsValue>> =
    payload => Promise.resolve(ok({ selection: payload.selection }))
  ...
  sessionRemotes(): RuntimeRemotes {
    return {
      ...
      session: {
        ...
        selectAgents: payload => this.record(
          'session.selectAgents',
          payload,
          this.onSelectAgents(payload),
        ),
