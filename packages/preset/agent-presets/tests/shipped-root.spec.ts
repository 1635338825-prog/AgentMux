...
describe('the shipped preset root', () => {
  ...
  it('enables web_fetch in each tool-bearing Web app preset', async () => {
    for (const id of ['cordis', 'multi-agent', 'ptc', 'standard']) {
  ...
  it('ships the multi-agent preset with local, Codex, Claude Code, Doubao Desktop, and Qwen delegation', async () => {
    const entries = await shippedEntries('multi-agent')
    expect(findEntry(entries, 'tool-subagent')?.disabled).not.toBe(true)
    expect(findEntry(entries, 'tool-subagent-codex')?.disabled).not.toBe(true)
    expect(findEntry(entries, 'tool-subagent-claude-code')?.disabled).not.toBe(true)
    expect(findEntry(entries, 'tool-subagent-doubao-desktop')?.disabled).not.toBe(true)
    expect(findEntry(entries, 'tool-subagent-qwen')?.disabled).not.toBe(true)
    expect(findEntry(entries, 'tool-jobs')?.disabled).not.toBe(true)
    expect(findEntry(entries, 'tool-workflow')?.disabled).not.toBe(true)
  })
