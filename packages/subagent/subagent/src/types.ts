...
export interface SubagentStartRequest {
  ...
  /**
   * Optional product-native model selected for an external Agent runtime.
   * Unlike `agentOptions`, this is interpreted only by the named provider
   * (for example a Codex, Claude Code, or Qwen CLI model id).
   */
...
export interface SubagentProvider {
  ...
  /** Product-native model ids this external provider accepts from delegation tools. */
  readonly backendModels?: readonly string[]
  /** Product-native model used when `backendModel` is omitted. */
  readonly backendModelDefault?: string
  /**
