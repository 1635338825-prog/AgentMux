...
export function apply(ctx: Context, config: Config): void {
  ...
  const install = (runtimeCtx: Context, modelSelectionPolicy: ModelSelectionPolicy | undefined): void => {
    ...
    const mount = (subagentProvider: SubagentProvider): void => {
      ...
      const backendModels = subagentProvider.backendModels ?? []
      const backendModelDefault = subagentProvider.backendModelDefault
      ...
      const disposeTool = runtimeCtx.tools.register(defineTool({
        ...
        description: wording.description + (backgroundEnabled
          ...
          + (backendModels.length === 0
            ? ''
            : ` Choose an optional product-native model with \`backend_model\`; omit it to use ${backendModelDefault ?? 'the provider default'}.`),
        parameters: {
          description: {
            type: 'string',
            required: true,
          ...
          ...backendModels.length === 0 ? {} : {
            backend_model: {
              ...
              enum: [...backendModels],
              description: `Product-native model for this Agent. Omit to use ${backendModelDefault ?? 'the provider default'}.`,
            },
          },
        ...
        async execute(args, exec) {
          ...
          const backendModel = (args as { backend_model?: unknown }).backend_model
          if (backendModel !== undefined && (
            typeof backendModel !== 'string' || !backendModels.includes(backendModel)
          )) {
            throw new Error(`backend_model must be one of: ${backendModels.join(', ')}`)
          }
          ...
          const request = {
            ...
            parent,
            ...typeof backendModel === 'string' ? { backendModel } : {},
