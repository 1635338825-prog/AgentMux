...
const DEFAULT_MODELS = ['sonnet', 'opus', 'haiku'] as const
...
export interface Config {
  ...
  model?: string
  /** Stable model aliases exposed to the browser's per-Agent selector. */
  models?: string[]
  /**
...
class ClaudeCodeProvider implements SubagentProvider {
  ...
  get backendModelDefault(): string {
    return this.config.model ?? 'the current Claude Code setting'
  }
  ...
  async start(request: ResolvedSubagentStartRequest) {
    ...
    const spec: ClaudeCodeRunSpec = {
      cwd,
      ...request.backendModel === undefined && this.config.model === undefined
        ? {}
        : { model: request.backendModel ?? this.config.model },
