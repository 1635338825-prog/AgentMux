...
export interface IConversation {
  ...
  /** Optional prompt-transform registry used by cross-cutting execution policies. */
  readonly submissions?: ConversationSubmissionTransforms
  /**
...
/** One browser-side transform applied only to the Host-bound prompt, never to the visible draft or echo. */
export type ConversationSubmissionTransform = (sessionId: SessionId, text: string) => string

/** Ordered named prompt transforms owned by the Conversation service. */
export interface ConversationSubmissionTransforms {
  register(name: string, transform: ConversationSubmissionTransform): () => void
  apply(sessionId: SessionId, text: string): string
}

class SubmissionTransforms implements ConversationSubmissionTransforms {
  private readonly transforms = new Map<string, ConversationSubmissionTransform>()

  register(name: string, transform: ConversationSubmissionTransform): () => void {
    if (this.transforms.has(name)) throw new Error(`conversation submission transform already registered: ${name}`)
  ...
  apply(sessionId: SessionId, text: string): string {
    let output = text
    for (const transform of this.transforms.values()) output = transform(sessionId, output)
    return output
  }
}
...
export class ConversationController extends Service implements IConversation {
  ...
  readonly blocks: ComposerBlocks
  readonly submissions: ConversationSubmissionTransforms = new SubmissionTransforms()
  ...
  async send(text: string): Promise<void> {
    ...
    const transformed = this.submissions.apply(session.sessionId, text)
    const result = await session.prompt([{ type: 'text', text: transformed }], 'queue')
  ...
  async sendSession(
    ...
    try {
      ...
      const uploaded = await this.serializeImages(attachments.map(attachment => attachment.file))
      const transformed = this.submissions.apply(session.sessionId, text)
      content = [...uploaded, ...(transformed === '' ? [] : [{ type: 'text' as const, text: transformed }])]
    } catch (error) {
      submission.abandon()
      throw error
    }
