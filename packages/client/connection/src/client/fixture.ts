...
function projectionValuesOf(log: readonly SessionEvent[]): Record<string, unknown> {
  ...
  values['agentSelection'] = agentSelectionProjectionOf(log)
...
function agentSelectionProjectionOf(log: readonly SessionEvent[]): Readonly<Record<string, string>> | null {
  const event = log.findLast(item => (item as { type: string }).type === 'agent/selection')
  return event === undefined
    ? null
    : (event as unknown as { data: { selection: Readonly<Record<string, string>> | null } }).data.selection
}
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const sessionApi: FixtureSessionApi = {
    ...
    selectAgents: (request) => {
      append(request.sessionId, { type: 'agent/selection', data: { selection: request.selection } })
      return sessionOk({ selection: request.selection })
    },
    prompt: (request) => {
  ...
  const rpc: ClientConnectionRpc = {
    call(channel, endpoint, payload, signal) {
      ...
      switch (endpoint) {
        ...
        case 'session/selectAgents': return sessionApi.selectAgents(
          request as Parameters<FixtureSessionApi['selectAgents']>[0],
        )
