...
import { writeAgentSelection } from './agent-selection.ts'
...
export const inject = ['commandUi', 'conversation', 'locale', 'sessions', 'slots', 'remote', 'remote.session']
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.inject(['conversation', 'slots', 'modelDirectories'], (scope: ClientContext) => {
    const models = scope.modelDirectories
    const sessions = scope.sessions
    scope.slots.inject('conversation.input.model', () => scope.slots.register({
      ...
      inject: (sessionId): ModelSelectInjected => {
        ...
        if (actx !== undefined) {
          ...
          const migrated = writeAgentSelection(draft, null)
          if (migrated !== draft) input.setDraft(migrated)
        }
        const agentSelection = scope.conversation.submissions === undefined
          ? undefined
          : models.agentSelectionFor(sessionId)
        return {
          available,
          directory: directory.store,
          ...agentSelection === undefined ? {} : {
            agentSelection,
            selectAgents: (selection: import('./agent-selection.ts').AgentSelection) => {
              void models.selectAgents(sessionId, selection)
            },
          },
          load: () => {
