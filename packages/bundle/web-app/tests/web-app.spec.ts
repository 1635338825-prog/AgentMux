...
beforeEach(() => {
  ...
  vi.stubEnv('SSH_TTY', '')
  internals.writeRuntimeLaunchUrl = vi.fn(async () => {})
})
...
afterEach(() => {
  ...
  internals.writeRuntimeLaunchUrl = originalWriteRuntimeLaunchUrl
...
const originalWriteRuntimeLaunchUrl = internals.writeRuntimeLaunchUrl
...
describe('web-app runtime glue', () => {
  it('mounts dist serving, prompt section, bash variables, and publishes the URL with the LAN snapshot', async () => {
    ...
    expect(internals.writeRuntimeLaunchUrl).toHaveBeenCalledWith(4567, 'http://127.0.0.1:4567/?token=test-token')
