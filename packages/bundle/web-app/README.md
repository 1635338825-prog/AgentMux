...
The URL line and browser handoff are readiness signals: supervisors RPC as soon as they observe the line, and a browser requests the page as soon as it opens, so both run only after the Loader tree settles and Connection authentication is available — or immediately in a hand-built tree without a Loader. At the same point, the authenticated loopback URL is atomically published as a mode-`0600` runtime handoff for local desktop clients. A tree disposed mid-boot announces nothing.
...
| File | Role |
|---|---|
| [`src/index.ts`](src/index.ts) | The `web-app` glue plugin: dist resolution, LAN trust sampling, prompt sections, bash variable, URL line, desktop runtime handoff, browser handoff |
