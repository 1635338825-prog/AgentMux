...
/** Publish the current process-token URL for trusted local desktop shells. */
async function writeRuntimeLaunchUrl(port: number, url: string): Promise<void> {
  const path = dshHomePath('runtime', `web-${String(port)}.json`)
  await writeFileAtomic(path, `${JSON.stringify({ version: 1, pid: process.pid, url })}\n`, {
    mode: 0o600,
    dirMode: 0o700,
  })
}
...
export const internals: {
  ...
  writeRuntimeLaunchUrl: (port: number, url: string) => Promise<void>
} = { resolveDistIndex, openBrowser, writeRuntimeLaunchUrl }
  ...
  if (config.printUrl || handoffBrowser) {
    ctx.inject(['connection'], (connectionCtx) => {
      ...
      const announceReady = (): void => {
        ...
        void internals.writeRuntimeLaunchUrl(port, authenticatedUrl).catch((error: unknown) => {
          const reason = error instanceof Error ? error.message : String(error)
          console.error(`web-app: could not publish the local desktop authentication handoff because ${reason}`)
        })
        if (config.printUrl) {
