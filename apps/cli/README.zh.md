...
运行命令时所在的目录将作为默认 workspace 根目录。`web`、`multi-agent`、`headless`、`sdk`、`sdk-minimal` 和 `acp` profile 在首次使用时会从随附模板自动初始化；其他任何 profile 都必须通过 `dsh plugin` 创建。`multi-agent` profile 会打开浏览器，并为其编排 preset 启用本地、Codex、Claude Code 与用户开启的豆包桌面版委派。
...
dsh --profile multi-agent           # browser orchestration with product delegates
...
`dsh.profile.bundles` 中列出的组合包先从 dsh 安装目录解析（`@deepseek-ai/dsh-base`、`@deepseek-ai/dsh-web-app`、`@deepseek-ai/dsh-multi-agent-app`、`@deepseek-ai/dsh-headless`、`@deepseek-ai/dsh-sdk-app`、`@deepseek-ai/dsh-sdk-minimal`、`@deepseek-ai/dsh-acp-app`），再从 profile 自身的 `node_modules` 解析；pnpm 会将树外插件安装到该目录。
