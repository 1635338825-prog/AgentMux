...
The invoking directory is the default workspace root. The `web`, `multi-agent`, `headless`, `sdk`, `sdk-minimal`, and `acp` profiles auto-initialize on first use from shipped templates; any other profile must be created through `dsh plugin`. The `multi-agent` profile opens the browser with local, Codex, Claude Code, and user-enabled Doubao Desktop delegation enabled for its orchestration preset.
