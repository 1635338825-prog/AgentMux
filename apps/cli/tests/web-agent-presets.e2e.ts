...
const DOUBAO_DESKTOP_PACKAGE_DIR = join(REPO_ROOT, 'packages/subagent/subagent-doubao-desktop')
const QWEN_PACKAGE_DIR = join(REPO_ROOT, 'packages/subagent/subagent-qwen')
...
describe('the shipped Web composition', () => {
  ...
  it('supplies every shipped preset, and only those, from the system root', async () => {
    ...
    expect(listed.map(preset => preset.id).sort()).toEqual(['cordis', 'minimal', 'multi-agent', 'ptc', 'standard'])
...
describe('product Bundle and user-preset intersection', () => {
  ...
  type Product = 'codex' | 'claude-code' | 'doubao-desktop' | 'qwen'
  ...
  async function bootProducts(installed: readonly Product[]): Promise<Context> {
    ...
    const packageDir = (product: Product): string => {
      if (product === 'codex') return CODEX_PACKAGE_DIR
      if (product === 'claude-code') return CLAUDE_CODE_PACKAGE_DIR
      if (product === 'doubao-desktop') return DOUBAO_DESKTOP_PACKAGE_DIR
      return QWEN_PACKAGE_DIR
    }
    const packageName = (product: Product): string => {
      if (product === 'codex') return '@deepseek-ai/dsh-subagent-codex'
      if (product === 'claude-code') return '@deepseek-ai/dsh-subagent-claude-code'
      if (product === 'doubao-desktop') return '@deepseek-ai/dsh-subagent-doubao-desktop'
      return '@deepseek-ai/dsh-subagent-qwen'
    }
          ...
          try {
            ...
            for (const productTool of productTools) {
              expect(toolParameterNames(productCtx, handle.agent, productTool)).toEqual([
                'description', 'prompt', 'backend_model', 'run_in_background',
              ])
            }
          } finally {
            await handle.dispose()
  ...
  it('composes the shipped multi-agent preset with every delegation target', async () => {
    const productCtx = await bootProducts(['codex', 'claude-code', 'doubao-desktop', 'qwen'])
    const spawn = vi.spyOn(productCtx.subprocess, 'spawn')
    try {
      const handle = await productCtx.agents.create({
        sessionId: SessionId(`preset-multi-agent-${randomUUID()}`),
        setup: agentCtx => productCtx.agentPresets.mount(agentCtx, 'multi-agent').then(() => undefined),
      })
      try {
        const tools = toolNames(productCtx, handle.agent)
        expect(tools).toEqual(expect.arrayContaining([
        ...
        expect(toolParameterNames(productCtx, handle.agent, 'subagent_codex')).toEqual([
          'description', 'prompt', 'backend_model', 'run_in_background',
        ])
        expect(toolParameterNames(productCtx, handle.agent, 'subagent_claude_code')).toEqual([
          'description', 'prompt', 'backend_model', 'run_in_background',
        ])
        expect(toolParameterNames(productCtx, handle.agent, 'subagent_doubao')).toEqual([
          'description', 'prompt', 'backend_model', 'run_in_background',
        ])
        expect(toolParameterNames(productCtx, handle.agent, 'subagent_qwen')).toEqual([
          'description', 'prompt', 'backend_model', 'run_in_background',
        ])
      } finally {
        await handle.dispose()
...
describe('a composition that configures its own preset roots', () => {
  ...
  it('keeps configured roots alongside the always-prepended shipped root', async () => {
    ...
    expect(listed.map(preset => preset.id).sort()).toEqual([
      'cordis', 'minimal', 'multi-agent', 'ptc', 'standard', 'team-spec',
    ])
